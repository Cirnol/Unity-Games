...
public class CodeEnemy : MonoBehaviour {
    ...
    public interface IEnemyTargetable {
        Vector3 GetPosition();
        void Damage(CodeEnemy attacker);
    }

    public static List<CodeEnemy> enemyList = new List<CodeEnemy>();

    public static CodeEnemy GetClosestEnemy(Vector3 position, float maxRange) {
        CodeEnemy closest = null;
        foreach (CodeEnemy enemy in enemyList) {
    ...
    public static CodeEnemy Create(Vector3 position) {
        Transform enemyTransform = Instantiate(GameAssets.i.pfEnemy, position, Quaternion.identity);

        CodeEnemy enemyHandler = enemyTransform.GetComponent<CodeEnemy>();
